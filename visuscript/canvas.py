...
class _Player:
    def __init__(self, scene: "Scene", print_initial):
        self._scene = scene
        self._first = print_initial
    
    def __lshift__(self, animation: Animation):
        if self._first and self._scene._embed_levels == 0:
            self._scene.print()
            self._first = False
...
class Scene(Canvas):
    ...
    * :class:`~visuscript.updater.Updater` objects with :code:`scene.updaters << updater`
    ...
    def __init__(self, print_initial=True, **kwargs):
        ...
        self._player = _Player(self, print_initial)
