...
class Shape:
    ...
    def __init__(self, drawable: "Drawable", transform: Transform = Transform()):
        ...
        self.circumscribed_radius: float = (
            drawable.circumscribed_radius * transform.scale.xy.max()
        )
...
class Drawable(ABC, Invalidatable):
    ...
    def __init__(
        ...
        *,
        ...
        stroke_width = (
            config.element_stroke_width
            if stroke_width is DEFER_TO_CONFIG
            else stroke_width
        )
        fill = config.element_fill if fill is DEFER_TO_CONFIG else fill

        self._transform: Transform = (
            Transform() if transform is None else Transform(transform)
        )
