...
class AnimationMetaClass(ABCMeta):
    ...
    def __new__(meta, name, bases, attrs):
        ...
        if inspect.signature(cls.__init__).parameters != inspect.signature(cls.__init_locker__).parameters:
            print(inspect.signature(cls.__init__), inspect.signature(cls.__init_locker__))
            raise TypeError(f"The '__init__' method and the '__init_locker__' method must have the exact same signature for class '{cls.__name__}', including type hints, parameter names, parameter order, and keyword argument default values. This error could result from overloading one but not the other.")
...
class NoAnimation(Animation):
    ...
    def __init_locker__(self, *, fps: int | ConfigurationDeference = DEFER_TO_CONFIG, duration: float | ConfigurationDeference = DEFER_TO_CONFIG):
...
class UpdaterAnimation(Animation):
    ...
    def __init_locker__(self, updater: Updater, *, duration: float | ConfigurationDeference = DEFER_TO_CONFIG): # type: ignore[reportIncompatibleMethodOverride]
...
class AlphaAnimation(Animation):
    ...
    @abstractmethod
    def __init_locker__(self, *, fps: int | ConfigurationDeference = DEFER_TO_CONFIG, duration: float | ConfigurationDeference = DEFER_TO_CONFIG, easing_function: Callable[[float], float] = sin_easing2): # type: ignore[reportIncompatibleMethodOverride]
        ...
