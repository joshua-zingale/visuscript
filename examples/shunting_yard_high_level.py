...
def main():
    ...
    tokens = tokenize(inp)
    input_organizer = GridOrganizer((1, len(tokens)), (1, SEPARATION)).set_transform(Transform(UP * FONT_SIZE*3 + LEFT * SEPARATION * (len(tokens)//2) - SEPARATION/2 + FONT_SIZE))
    input_organizer.organize(tokens)
    scene << tokens
    scene << Text("Infix Notation", FONT_SIZE*2/3).set_anchor(Anchor.BOTTOM_LEFT).translate(tokens[0].global_shape.top_left + UP*FONT_SIZE/2)

    shunting_arrow = Arrow(
        source=(tokens[0].global_shape.center + tokens[-1].global_shape.center)/2 + DOWN * FONT_SIZE,
        destination=(tokens[0].global_shape.center + tokens[-1].global_shape.center)/2 + DOWN * FONT_SIZE*2
        ).set_opacity(0.0)
    shunting_text = Text("Shunting Yard Algorithm", font_size=FONT_SIZE//2).set_anchor(Anchor.LEFT).translate(shunting_arrow.shape.right + RIGHT*FONT_SIZE/2).set_opacity(0.0)
    polish_text = Text("Reverse Polish Notation", FONT_SIZE*2/3).set_anchor(Anchor.BOTTOM_LEFT).translate(tokens[0].global_shape.top_left + DOWN*FONT_SIZE*3 + UP*FONT_SIZE/2).set_opacity(0.0)
    ...
    polish_organizer = GridOrganizer((1, len(tokens)), (1, SEPARATION)).set_transform(Transform(LEFT * SEPARATION * (len(tokens)//2) - SEPARATION/2 + FONT_SIZE))
    scene << polish_tokens
    ...
    evaluation_organizer = GridOrganizer((1, len(tokens)), (1, SEPARATION)).set_transform(Transform(DOWN * FONT_SIZE * 3 + LEFT * SEPARATION * (len(tokens)//2) - SEPARATION/2 + FONT_SIZE))
    scene << evaluation_tokens
    expression_text = Text("Expression Evaluation", FONT_SIZE*2/3).set_anchor(Anchor.BOTTOM_LEFT).translate(tokens[0].global_shape.top_left + DOWN*FONT_SIZE*6 + UP*FONT_SIZE/2).set_opacity(0.0)
...
def organizer_animation(objects: Sequence[HasTransform], organizer: Organizer):
    return AnimationBundle(*map(lambda v: TransformAnimation(v[0].transform, v[1]), zip(objects, organizer)))
...
def evaluate_step(tokens: list[Union["Operator", "Value"]]):
    ...
    value = Value(f"{result:.3}".rstrip("0").rstrip(".")).set_opacity(0.0).set_transform(lvalue.transform)
    operator = tokens[i]
    return AnimationSequence(
        AnimationBundle(*map(lambda v: RgbAnimation(v.fill, "gold"), tokens[i-2:i+1])),
        AnimationBundle(
            *map(lambda v: ScaleAnimation(v.transform, 0.5), tokens[i-2:i+1]),
            TranslationAnimation(lvalue.transform, lvalue.transformed_shape.center + DOWN * FONT_SIZE + LEFT * lvalue.transformed_shape.width/3 + LEFT * operator.transformed_shape.width/4),
            TranslationAnimation(operator.transform, lvalue.transformed_shape.center + DOWN * FONT_SIZE),
            TranslationAnimation(rvalue.transform, lvalue.transformed_shape.center + DOWN * FONT_SIZE + RIGHT * rvalue.transformed_shape.width/3 + RIGHT * operator.transformed_shape.width/4)
            ),
