...
i = 2
j = 1
def save_slide(c: Canvas):
    global i
    global j
    save_svg(c, f"./midterm_slides/{i-1}-{j}.svg")
    i += 1
    j = 1
def save_subslide(c: Canvas):
    global i
    global j
    save_svg(c, f"./midterm_slides/{i-1}-{j}.svg")
    j += 1
...
c << get_multiline_texts(curr_code(), 7, anchor=Drawable.TOP_LEFT).set_transform([c.x(0.01),c.y(0)])
...
s << get_multiline_texts(curr_code(), 7, anchor=Drawable.TOP_LEFT).set_transform([c.x(0.01),c.y(0)])
...
c << Text(text="Data structure updates can be auto visualized by overloading opperators.", font_size=15, anchor=Drawable.LEFT, transform=[c.x(0.01),c.y(0.85)])

save_slide(c)

## SLIDE
c = Canvas()
arrow = lambda : Drawing(path=Path().L(25,0).l(0,-3).l(2,3).l(-2,3).l(0,-3), anchor=Drawable.LEFT).set_fill('off_white')
fs = 10
...
c << Text(text="Python", font_size=fs, anchor=Drawable.LEFT).set_transform([-150,0]).with_child(arrow().set_transform([6*fs,0])
                                                  .with_child(
                                                      Text(text="svg", font_size=fs,anchor=Drawable.LEFT).set_transform([6*fs,0])
                                                        .with_child(arrow().set_transform([3*fs,0])
                                                            .with_child(Text(text="png", font_size=fs,anchor=Drawable.LEFT).set_transform([6*fs,0])
                                                                        .with_child(arrow().set_transform([3*fs,0])
                                                                            .with_child(Text(text="mp4", font_size=fs,anchor=Drawable.LEFT).set_transform([6*fs,0])
                                                                        ))))))
save_slide(c)
...
## SLIDE
c = Canvas()
c << get_multiline_texts(curr_code(), 10, anchor=Drawable.TOP_LEFT).set_transform([c.x(0.01),c.y(0)])
