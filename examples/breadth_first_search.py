...
def main():
    ...
    # Build graph
    random_adjacency_matrix = [[0 for _ in range(N)] for _ in range(N)]
    ...
    for distance, source, destination in bfs(nodes, edges):
        sequence << AnimationBundle(
            ...
            RgbAnimation(edges.get_edge(source, destination).stroke, "red")
            if source
            else None,
        )
...
def get_nodes_and_edges(adjacency_matrix):
    ...
    nodes = [
        GraphNode(chr(ord("A") + i)).translate(
            random.randrange(-30, 30), random.randrange(-20, 20)
        )
...
def get_graph_updater(nodes: list[GraphNode], edges: Edges):
    def graph_physics(t: float, dt: float, edges=edges):
        positions = {
            node: pos
            for node, pos in zip(
                nodes, map(lambda x: x.transformed_shape.center, nodes)
            )
        }
        ...
        for node1 in nodes:
            ...
            for node2 in nodes:
                ...
                if edges.connected(node1, node2):
                    force += repulse_force(
                        node1_pos, node2_pos, REPULSION_CONSTANT / 16
                    )
            ...
            for start, end in lines:
                force += line_repulsion_force(
                    node1_pos, start, end, LINE_REPULSION_CONSTANT
                )
...
def line_repulsion_force(
    loc: Vec2, line_start: Vec2, line_end: Vec2, constant: float
) -> Vec2:
    ...
    return (
        constant * (Vec2(0, pos.y) @ basis_vectors) / max(distance_from_line, 1e-8) ** 2
    )
...
def bfs(
    ...
    while queue:
        ...
        next_nodes = list(
            filter(lambda x: edges.connected(destination, x) and x not in queued, nodes)
        )
