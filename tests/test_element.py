...
class TestElement(VisuscriptTestCase):
    ...
    def test_functional_add_children(self):
        ...
        self.assertVecAlmostEqual(child.transform.translation, parent.shape.bottom_right.extend(0))
        self.assertTrue(child in parent)

        children = [self.MockElement(10,10) for _ in range(5)]
        parent.add_child(lambda p: (child.translate(*p.shape.right + Vec2(10*i, 0)) for i, child in enumerate(children)))
        self.assertSetEqual(set(children).union(child), set(parent.iter_children()))
        for i, child in enumerate(children):
            self.assertVecAlmostEqual(child.transform.translation, parent.shape.right.extend(0) + Vec3(10*i, 0, 0))
