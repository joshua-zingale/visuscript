...
class TestAnimatedList(VisuscriptTestCase):
    ...
    def test_quadratic_swap_animation(self):
        ...
        self.assertAlmostEqual(
            array.elements[0].transform.translation.x, (loc1.x + loc2.x) / 2
        )
        self.assertAlmostEqual(
            array.elements[-1].transform.translation.x, (loc1.x + loc2.x) / 2
        )
        self.assertLessEqual(
            array.elements[-1].global_shape.bottom.y,
            array.elements[0].global_shape.top.y,
        )
...
class TestAnimatedArray(VisuscriptTestCase):
    def test_array_creation(self):
        ...
        self.assertGreaterEqual(
            len(get_elements_of_type(array.auxiliary_elements, Rect)), len(data)
        )
